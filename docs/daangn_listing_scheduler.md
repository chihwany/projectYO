...
전국 279개 구/군의 최신 매물을 1분 주기로 수집하여 새 매물을 감지하고, 사용자가 등록한 키워드와 매칭되는 매물이 발견되면 알림을 발송한다.
...
### seen_ids 기반 감지 (createdAt 시간 필터 미사용)

당근 API는 구/군별 최신 매물 ~300건을 반환하며, 이 목록은 **실시간이 아닌 캐시 기반 로테이션** 방식으로 갱신된다. 실측 결과, 인기 지역(강남구)에서도 가장 최근 매물의 `createdAt`이 ~8분 이상 전이었다.

따라서 `createdAt` 기반 시간 필터(예: "1분 이내 등록") 대신, **Redis seen_ids 비교**를 통해 새 매물을 감지한다:

```
이전 수집에 없던 매물 ID = 새 매물
```

이 방식은 API의 캐시 갱신 주기에 관계없이, 매 수집마다 새로 나타난 매물을 안정적으로 감지한다.
...
매 1분마다:
  ...
  [4단계] 키워드 매칭 및 알림
    ...
    └─ 새 매물의 title + content에 키워드 포함 여부 확인
    └─ 매칭된 사용자에게 FCM 알림 발송
    └─ 알림 이력 DB 저장
```
...
```
seen_ids 기반으로 감지된 새 매물의 (title + content)에 사용자 등록 키워드가 포함되어 있으면 매칭
- 대소문자 구분 없음 (case-insensitive)
- 부분 일치 (contains)
- 새 매물 전체 대상 (createdAt 시간 필터 없음)
```
...
```
전국 매물 76,062건 중 "닌텐도" 매칭: 248건 (전체 매물 대상)
1분 간격 수집 시 새 매물: ~1건 (API 캐시 로테이션이 느림)
```
